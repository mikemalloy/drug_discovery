...
import { CheckCircle2, XCircle, AlertTriangle, Shield, ShieldAlert, ShieldCheck, Crosshair, HelpCircle, Sparkles } from 'lucide-react'
...
function AiSummarySection({
  ...
  // Nothing to show and not loading: render nothing (e.g. summary disabled).
  if (!loading && (!summary || !summary.available)) {
    if (summary && !summary.available) {
      return (
        <div>
          <h2 className="text-sm font-semibold text-foreground mb-4 tracking-tight flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-accent" />
            AI Summary
          </h2>
          <p className="text-sm text-muted-foreground">
            Summary unavailable{summary.reason ? ` (${summary.reason})` : ''}. The
    ...
    <div>
      ...
      {loading ? (
        <div className="flex items-center gap-3 text-sm text-muted-foreground">
          <span className="inline-block h-3 w-3 rounded-full bg-accent/60 animate-pulse" />
          ...
          {summary?.governance && (
            <details className="mt-5 pt-4 border-t border-border/50">
              <summary className="text-xs text-muted-foreground cursor-pointer select-none hover:text-foreground">
                Generated by {summary.governance.model} · click for audit metadata
              </summary>
              ...
              <p className="mt-3 text-xs text-muted-foreground leading-relaxed">
                Interpretation of model outputs only. Not a clinical, dosing, or
                fitness-for-use recommendation.
              </p>
            </details>
          )}
        </div>
...
export function ResultsDisplay({ data, summary, summaryLoading }: ResultsDisplayProps) {
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Grounded AI summary — plain-language interpretation of the numbers above */}
      <AiSummarySection summary={summary} loading={summaryLoading} />
